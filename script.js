...
// Navegação: rolagem suave para âncoras (#), navegação normal para páginas
document.querySelectorAll('.nav-links a').forEach(link => {
    link.addEventListener('click', (e) => {
        const href = link.getAttribute('href');
        if (href.startsWith('#')) {
            // Rolagem suave para âncoras (ex.: carrinho)
            e.preventDefault();
        ...
        // Para links de páginas (ex.: produtos.html), deixa navegar normalmente
        navLinks.classList.remove('active'); // Fecha menu mobile sempre
    });
});

// Renderizar produtos (só na página de produtos)
function renderProducts(filteredProducts = products) {
    const grid = document.getElementById('product-grid');
    if (grid) { // Verifica se o elemento existe na página
        grid.innerHTML = '';
...
// Carrossel de destaques (só na página inicial)
function renderHighlights() {
    const carousel = document.getElementById('carousel');
    if (carousel) { // Verifica se o elemento existe na página
...
function removeFromCart(index) {
    ...
    localStorage.setItem('cart', JSON.stringify(cart));
    updateCartCount();
    if (carrinhoLink) carrinhoLink.click(); // Reabre modal
}
...
// Barra de pesquisa (só na página de produtos)
...
// Filtros (só na página de produtos)
const categoriaSelect = document.getElementById('categoria');
const precoSelect = document.getElementById('preco');
if (categoriaSelect && precoSelect) {
...
// Formulário de contato (só na página de contato)
...
// Inicializar (dependendo da página)
renderProducts(); // Renderiza produtos se o grid existir
renderHighlights(); // Renderiza destaques se o carousel existir

console.log('Página carregada. Grid existe?', document.getElementById('product-grid'));
